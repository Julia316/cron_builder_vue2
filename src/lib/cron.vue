  ...
  <div class="cron_builder">
    <ul class="nav nav-tabs">
      <li class="nav-item" v-for="d in headers" :key="d">
        <a
          class="nav-link "
          :class="selectedTab == d ? 'active' : ''"
          v-on:click="changeTab(d)"
        >
          {{ translate(d) }}
        </a>
      </li>
    </ul>
    <div class="cron_builder_bordering">
      <Minutes
        v-if="selectedTab == headerValues.MINUTES"
        :value="value"
        @change-val="changeVal"
      />
      <Hourly
        v-if="selectedTab == headerValues.HOURLY"
        :value="value"
        @change-val="changeVal"
      />
      <Daily
        v-if="selectedTab == headerValues.DAILY"
        :value="value"
        @change-val="changeVal"
      />
      <Weekly
        v-if="selectedTab == headerValues.WEEKLY"
        :value="value"
        @change-val="changeVal"
      />
      <Monthly
        v-if="selectedTab == headerValues.MONTHLY"
        :value="value"
        @change-val="changeVal"
      />
      <Custom
        v-if="selectedTab == headerValues.ADVANCED"
        :value="value"
        @change-val="changeVal"
      />
    </div>
...
export default {
  ...
  methods: {
    ...
    getDefaultVal(tab) {
      let defaultValue = this.metadata.find((me) => me.name == tab).initialCron;
      return [...defaultValue];
  ...
  computed: {
    getResultText: function() {
      let val = cronstrue.toString(this.cron, {
        throwExceptionOnParseError: false,
        locale: this.locale ? this.locale : "en",
      });
      if (val.search("undefined") === -1) {
